...
-- ---------------------------------------------------------------------------
-- 5) Preço dinâmico: os limites de demanda.
-- ---------------------------------------------------------------------------
--
-- O multiplicador em si é calculado na hora, a partir de quantos chamados estão
-- abertos e quantos técnicos estão disponíveis no recorte — não é dado
-- guardado, porque envelhece em minutos. O que É dado, e por isso mora aqui,
-- são os limites dentro dos quais ele pode variar: sem teto, um pico de demanda
-- multiplicaria o preço sem freio.
--
-- Reaproveita pricing_rules em vez de criar tabela nova: 'demand' é mais uma
-- natureza de regra, com a mesma resolução por especificidade que as outras
-- quatro já usam. min/max são o multiplicador em centésimos — 100 é "preço
-- cheio, sem ajuste".
ALTER TABLE pricing_rules DROP CONSTRAINT IF EXISTS pricing_rules_kind_check;

ALTER TABLE pricing_rules
    ADD CONSTRAINT pricing_rules_kind_check
    CHECK (kind IN ('share','fee','promo','bounds','demand'));

-- ADD CONSTRAINT não aceita IF NOT EXISTS, e a migration precisa poder rodar
-- duas vezes sem quebrar — é o que o resto do arquivo garante com IF NOT
-- EXISTS. Daí o DROP antes.
ALTER TABLE pricing_rules DROP CONSTRAINT IF EXISTS pricing_rules_demand_precisa_limite;

ALTER TABLE pricing_rules
    ADD CONSTRAINT pricing_rules_demand_precisa_limite
    CHECK (kind <> 'demand' OR (min_cents IS NOT NULL AND max_cents IS NOT NULL));

-- Sem regra cadastrada não há ajuste nenhum: o padrão global trava o
-- multiplicador em 1,00 para os dois lados, e quem quiser preço dinâmico
-- cadastra o recorte e a folga que aceita.
INSERT INTO pricing_rules(kind,min_cents,max_cents,note)
SELECT 'demand',100,100,'padrão: sem ajuste por demanda'
WHERE NOT EXISTS (SELECT 1 FROM pricing_rules WHERE kind='demand');
