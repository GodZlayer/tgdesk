...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("TGDesk".to_owned());
...
impl Config {
    ...
    pub fn get_options() -> HashMap<String, String> {
        ...
        let saved = CONFIG2.read().unwrap().options.clone();
        res.extend(saved.clone());
        res.extend(OVERWRITE_SETTINGS.read().unwrap().clone());
        res.extend(
            saved
                .into_iter()
                .filter(|(k, _)| is_tgdesk_service_managed_option(k)),
        );
        res
    }
    ...
    fn purify_options(v: &mut HashMap<String, String>) {
        v.retain(|k, v| {
            is_tgdesk_service_managed_option(k)
    ...
    pub fn get_option(k: &str) -> String {
        if is_tgdesk_service_managed_option(k) {
            if let Some(value) = CONFIG2.read().unwrap().options.get(k) {
                return value.clone();
            }
        }
        get_or(
...
#[inline]
fn is_tgdesk_service_managed_option(k: &str) -> bool {
    matches!(
        k,
