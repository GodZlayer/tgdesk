...
#[cfg(windows)]
struct TgdeskServiceGenerationGuard(winapi::um::winnt::HANDLE);

#[cfg(windows)]
impl TgdeskServiceGenerationGuard {
    fn acquire() -> Option<Self> {
        use std::os::windows::ffi::OsStrExt;
        let name: Vec<u16> = std::ffi::OsStr::new("Global\\TGDeskServiceGeneration")
            .encode_wide()
            .chain(std::iter::once(0))
            .collect();
        let handle = unsafe { CreateMutexW(std::ptr::null_mut(), FALSE, name.as_ptr()) };
        if handle.is_null() {
            return None;
        }
...
#[cfg(windows)]
impl Drop for TgdeskServiceGenerationGuard {
    fn drop(&mut self) {
        unsafe {
            ReleaseMutex(self.0);
            CloseHandle(self.0);
...
pub fn core_main() -> Option<Vec<String>> {
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        if args[0] == "--remove" {
            ...
            // SCM may report the previous generation stopped before its
            // process has finished killing the Host/Technician children.
            // Serialize generations so the replacement cannot race the old
            // Host's global singleton mutex and exit without a tunnel.
            #[cfg(windows)]
            let _tgdesk_service_generation = TgdeskServiceGenerationGuard::acquire();
            #[cfg(windows)]
            let mut tgdesk_children = if let Ok(exe) = std::env::current_exe() {
                let mut children = Vec::new();
                if let Ok(child) = std::process::Command::new(&exe)
                    .args(["--tgdesk-host", "--server", "http://168.232.199.161:8090"])
                    .spawn()
                {
                    children.push(child);
                }
                if let Ok(child) = std::process::Command::new(&exe)
                    .args([
                        "--tgdesk-technician-service",
                        "--server",
                        "http://168.232.199.161:8090",
                    ])
                    .spawn()
                {
                    children.push(child);
            ...
            // VPN/telemetria pertencem ao servico na sessao 0, mas o tray
            // obrigatorio pertence a sessao interativa. Este supervisor
            // tambem recupera o icone apos logon, reinicio do Explorer ou
            // encerramento inesperado do processo --tray.
            #[cfg(windows)]
            if let Ok(exe) = std::env::current_exe() {
                std::thread::spawn(move || loop {
                    if let Err(err) = crate::platform::ensure_tray_in_active_session(
                        exe.to_string_lossy().as_ref(),
                    ) {
                        log::warn!("TGDesk tray supervisor: {err}");
                    }
                    std::thread::sleep(std::time::Duration::from_secs(2));
                });
            }
            crate::start_os_service();
            // The helper processes are part of the TGDesk service lifecycle.
            // Leaving them orphaned on SERVICE_CONTROL_STOP made every service
            // restart accumulate another technician tunnel process.
            #[cfg(windows)]
            for child in &mut tgdesk_children {
                let _ = child.kill();
                let _ = child.wait();
            }
            return None;
...
fn is_cli_setting_change_disabled() -> bool {
    // TGDesk manages the private rendezvous and unattended-access policy from
    // its elevated Windows service. The visual settings remain hidden, but
    // installed/elevated management commands must stay available so the agent
    // can atomically apply and verify those server-owned values.
    false
}
