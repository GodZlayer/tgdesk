            ...
            tokio::select! {
                ...
                Some(data) = rx_from_cm.recv() => {
                    match data {
                        ...
                        ipc::Data::CmErr(e) => {
                            if e != "expected" {
                                // TGDesk unattended sessions are authorized by
                                // the control server before reaching this
                                // point. The legacy local connection-manager
                                // window is optional and must not tear down an
                                // already authorized session when no user UI is
                                // running (service/pre-login/UAC scenarios).
                                if conn.authorized {
                                    log::warn!(
                                        "connection manager unavailable; keeping authorized TGDesk session: {}",
                                        e
                                    );
                                    continue;
                                }
            ...
            match msg.union {
                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::ChatMessage(c)) => {
                        const ANNOTATION_PREFIX: &str = "__TGDESK_ANNOTATION__:";
                        if let Some(payload) = c.text.strip_prefix(ANNOTATION_PREFIX) {
                            #[cfg(not(any(target_os = "android", target_os = "ios")))]
                            if let Ok(event) =
                                serde_json::from_str::<crate::whiteboard::CustomEvent>(payload)
                            {
                                let key = crate::whiteboard::get_key_cursor(self.inner.id);
                                crate::whiteboard::register_whiteboard(key.clone());
                                crate::whiteboard::update_whiteboard(key, event);
                            }
                        } else {
