...
/// Garante um unico tray TGDesk na sessao interativa ativa. O servico chama
/// esta funcao continuamente porque a sessao 0 nao pode hospedar icones da
/// area de notificacao e o usuario pode entrar, sair ou reiniciar o Explorer.
pub fn ensure_tray_in_active_session(exe: &str) -> ResultType<bool> {
    ...
    let already_running = crate::platform::get_pids_of_process_with_args(
        &app_exe_name,
        &["--tray"],
    )
    .into_iter()
    ...
    run_exe_in_session(exe, vec!["--tray"], session_id, false)?;
    Ok(true)
}
...
/// Closes the interactive TGDesk window (if one is open) without touching
/// the Windows service or its Host/Technician-service children — lets the
/// tray "Close window" option restart just the UI, unlike "Stop service"
