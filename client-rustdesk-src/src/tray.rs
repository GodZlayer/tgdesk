...
pub fn start_tray() {
    ...
    // O processo --tray precisa permanecer dono do event loop. Executar
    // make_tray() numa thread destacada e retornar encerrava o processo logo
    // depois, destruindo o icone que acabara de ser criado.
    #[cfg(windows)]
    {
        let max_retries = 3;
            ...
            match make_tray() {
                ...
                Err(e) => {
                    if attempt < max_retries - 1 {
                        log::warn!(
                            "Tray initialization failed (attempt {}): {}, retrying...",
                            attempt + 1,
                            e
                        );
                        ...
                        log::error!(
                            "Failed to create tray after {} attempts: {}",
                            max_retries,
                            e
                        );
                    }
...
fn make_tray() -> hbb_common::ResultType<()> {
    ...
    #[cfg(not(target_os = "macos"))]
    {
        // TGDesk embeds its own multi-resolution icon; do not reuse RustDesk
        // branding when the optional runtime asset is absent.
    ...
    // Reinicia só a janela interativa (fecha o processo do app aberto), sem
    // tocar no serviço nem nos filhos Host/Technician-service que ele
    // supervisiona — diferente de "Stop service", que derruba tudo isso e
    // pode deixar processos orfaos se a queda nao for graciosa.
    #[cfg(windows)]
    let close_window_i = Some(MenuItem::new(
        translate("Close window".to_owned()),
        true,
        None,
    ));
    #[cfg(not(windows))]
    let close_window_i: Option<MenuItem> = None;
    if let Some(quit_i) = &quit_i {
        if let Some(close_window_i) = &close_window_i {
            tray_menu
                .append_items(&[&open_i, close_window_i, quit_i])
                .ok();
        } else {
            tray_menu.append_items(&[&open_i, quit_i]).ok();
        }
    } else if let Some(close_window_i) = &close_window_i {
        tray_menu.append_items(&[&open_i, close_window_i]).ok();
    } else {
    ...
    event_loop.run(move |event, _, control_flow| {
        *control_flow = ControlFlow::WaitUntil(
        ...
        if let Ok(event) = menu_channel.try_recv() {
            #[cfg(windows)]
            if let Some(close_window_i) = &close_window_i {
                if event.id == close_window_i.id() {
                    crate::platform::close_main_window();
                }
            }
            if let Some(quit_i) = &quit_i {
...
fn load_icon_from_asset() -> Option<image::DynamicImage> {
    ...
    if let Some(local_app_data) = std::env::var_os("LOCALAPPDATA") {
        let branded = std::path::PathBuf::from(local_app_data)
            .join("TGDesk")
            .join("branding")
            .join("favicon.ico");
        if branded.exists() {
            if let Ok(image) = image::open(branded) {
                return Some(image);
            }
        }
    }
    ...
    if let Some(program_data) = std::env::var_os("ProgramData") {
        let branded = std::path::PathBuf::from(program_data)
            .join("TGDesk")
            .join("branding")
            .join("favicon.ico");
        if branded.exists() {
