          ...
          Material(
            ...
            child: const TabBar(tabs: [
              ...
              Tab(
                  ...
                  text: 'Peças / consumíveis'),
            ]),
          ),
          const Padding(
            padding: EdgeInsets.fromLTRB(
                TgdeskSpacing.md, TgdeskSpacing.sm, TgdeskSpacing.md, 0),
            child: Text(
                'Use estas listas para organizar chamados e OS. Valores mínimos e máximos por região são definidos somente em Precificação.'),
          ),
          const Expanded(
...
class _AdminOsCatalogTabState extends State<AdminOsCatalogTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final consumables = _channel.parts
    ...
    return Scaffold(
      floatingActionButton: isServices
          ...
          : Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              FloatingActionButton.extended(
                  heroTag: 'novo-consumivel',
                  icon: const Icon(Icons.cable_outlined),
                  label: const Text('Consumível'),
                  onPressed: () => _editPart(null, initialKind: 'consumable')),
            ]),
      body: ListView(
          ...
          if (isServices) ...[
            if (items.isEmpty)
              const Padding(
                  ...
                  child: Text('Nenhum tipo de serviço cadastrado.'))
            else
            ...
            if (parts.isEmpty)
              const Padding(
                  padding: EdgeInsets.symmetric(vertical: TgdeskSpacing.sm),
                  child: Text('Nenhum tipo de peça cadastrado.'))
            else
              ...parts.map(_partCard),
            const SizedBox(height: TgdeskSpacing.lg),
            Text('Tipos de consumível',
                style: Theme.of(context).textTheme.titleSmall),
            if (consumables.isEmpty)
              const Padding(
                  padding: EdgeInsets.symmetric(vertical: TgdeskSpacing.sm),
                  child: Text('Nenhum tipo de consumível cadastrado.'))
            else
  ...
  Widget _serviceCard(Map<String, dynamic> service) {
    ...
    return Card(
      ...
      subtitle: Text(
          '${_scope(service)} · ${service['duration_min']} min${manual ? ' · com manual' : ''}'),
      enabled: active,
  ...
  Widget _ticketTypeDropdown(String? value, ValueChanged<String?> onChanged) =>
      DropdownButtonFormField<String?>(
        value: value,
        decoration:
            const InputDecoration(labelText: 'Vale para o tipo de chamado'),
        items: [
          const DropdownMenuItem(value: null, child: Text('Todos os tipos')),
          for (final type in _channel.ticketTypes)
            DropdownMenuItem(
  ...
  Future<void> _editPart(Map<String, dynamic>? part,
      {String initialKind = 'part'}) async {
    ...
    final ok = await showDialog<bool>(
        ...
        builder: (ctx) => StatefulBuilder(
            builder: (ctx, setLocal) => AlertDialog(
                  title: Text(part == null
                      ? 'Novo tipo de $typeLabel'
                      : 'Editar tipo de $typeLabel'),
                  content: SizedBox(
                      width: 420,
                      child: SingleChildScrollView(
                        ...
                        TextField(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'SKU',
                                helperText:
                                    'Identidade do item; não muda depois')),
                        TextField(
                            controller: label,
                            decoration:
                                const InputDecoration(labelText: 'Rótulo')),
                        TextField(
                            controller: unit,
                            decoration:
                                const InputDecoration(labelText: 'Unidade')),
                        const SizedBox(height: TgdeskSpacing.sm),
                        _ticketTypeDropdown(ticketType,
                            (value) => setLocal(() => ticketType = value)),
                        SwitchListTile(
                            contentPadding: EdgeInsets.zero,
                            title: const Text('Exige foto da nota fiscal'),
                            value: requiresInvoice,
                            onChanged: (value) =>
                                setLocal(() => requiresInvoice = value)),
                        SwitchListTile(
                            contentPadding: EdgeInsets.zero,
  ...
  Future<void> _editService(Map<String, dynamic>? service) async {
                      ...
                      child: SingleChildScrollView(
                        ...
                        TextField(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'Chave',
                                helperText:
                                    'Sem espaço nem barra; não muda depois')),
                        TextField(
                            controller: label,
                            decoration:
                                const InputDecoration(labelText: 'Rótulo')),
                        ...
                        TextField(
                            ...
                            decoration: const InputDecoration(
                                ...
                                helperText:
                                    'Material que o técnico consulta antes de executar')),
                        const SizedBox(height: TgdeskSpacing.sm),
                        _ticketTypeDropdown(ticketType,
                            (value) => setLocal(() => ticketType = value)),
                        DropdownButtonFormField<String?>(
                            value: mode,
                            decoration: const InputDecoration(
                                labelText: 'Modalidade da OS'),
                            items: const [
                              DropdownMenuItem(
                                  value: null, child: Text('As duas')),
                              DropdownMenuItem(
                                  value: 'virtual', child: Text('Remoto')),
                              DropdownMenuItem(
                                  value: 'onsite', child: Text('Presencial'))
                            ],
                            onChanged: (value) => setLocal(() => mode = value)),
                        SwitchListTile(
                            contentPadding: EdgeInsets.zero,
                            title: const Text('Ativo'),
                            value: active,
    ...
    await _run(() => TgdeskApi.saveService({
          'key': key.text.trim(),
          'label': label.text.trim(),
          'duration_min': int.tryParse(duration.text.trim()) ?? 60,
