...
class _ConnectionTabPageState extends State<ConnectionTabPage> {
  ...
  // TGDesk opens remote sessions as an embedded route inside the main
  // window (see lib/tgdesk/remote_session_page.dart, params['embedded']),
  // not as a real RustDesk multi-window. In that mode the RustDesk
  // DesktopTab (tab strip + "+" add-connection button + window buttons) is
  // redundant with TGDesk's own floating RemoteToolbar (see remote_page.dart,
  // tgdeskMode) and visually overlaps it, producing a double bar. Render
  // just the session content when embedded, and keep the original
  // DesktopTab-wrapped behavior for the native RustDesk multi_window flow.
  bool get _tgdeskEmbedded => widget.params['embedded'] == true;
  ...
  void onRemoveId(String id) async {
    if (tabController.state.value.tabs.isEmpty) {
      if (widget.params['embedded'] == true) {
