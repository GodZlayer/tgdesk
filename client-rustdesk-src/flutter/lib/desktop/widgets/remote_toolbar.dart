...
class _RemoteToolbarState extends State<RemoteToolbar> {
  ...
  Widget _buildToolbar(
    ...
    if (!widget.tgdeskMode && !isWebDesktop) {
    ...
    if (widget.tgdeskMode &&
        widget.captureSystemKeys != null &&
        widget.toggleSystemKeys != null) {
      toolbarItems.add(_SystemKeysMenu(
    ...
    if (!widget.tgdeskMode && widget.ffi.connType == ConnType.defaultConn) {
      toolbarItems.add(_KeyboardMenu(id: widget.id, ffi: widget.ffi));
    }
    if (!widget.tgdeskMode) {
      toolbarItems.add(_ChatMenu(id: widget.id, ffi: widget.ffi));
    }
    if (!widget.tgdeskMode && !isWeb) {
...
class _SystemKeysMenu extends StatelessWidget {
  const _SystemKeysMenu({
    required this.captureSystemKeys,
    required this.onPressed,
  });
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final remote = captureSystemKeys.value;
      return _IconMenuButton(
        icon: Icon(
          remote ? Icons.keyboard_tab : Icons.keyboard,
          size: _ToolbarTheme.buttonSize,
          color: Colors.white,
        ),
        tooltip: remote
            ? 'Atalhos do Windows: computador remoto (Ctrl+Shift+I)'
            : 'Atalhos do Windows: este computador (Ctrl+Shift+I)',
        onPressed: onPressed,
        color: remote ? _ToolbarTheme.blueColor : _ToolbarTheme.inactiveColor,
        hoverColor: remote
            ? _ToolbarTheme.hoverBlueColor
            : _ToolbarTheme.hoverInactiveColor,
      );
