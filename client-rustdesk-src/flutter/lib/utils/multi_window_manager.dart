...
class RustDeskMultiWindowManager {
  ...
  final List<int> _tgdeskHubWindows = List.empty(growable: true);
  ...
  /// Abre a janela do TGDesk Hub (Admin/Técnico) — mesmo processo, mesmo
  /// executável, uma janela nativa a mais (igual RustDesk já faz para
  /// sessões remotas, transferência de arquivo etc.).
  Future<MultiWindowCallResult> newTgdeskHub() async {
    if (_tgdeskHubWindows.isNotEmpty) {
      final windowId = _tgdeskHubWindows.first;
      await WindowController.fromWindowId(windowId).show();
      registerActiveWindow(windowId);
      return MultiWindowCallResult(windowId, null);
    }
    final params = {'type': WindowType.TgdeskHub.index};
    final windowController = await DesktopMultiWindow.createWindow(jsonEncode(params));
    final windowId = windowController.windowId;
    windowController
    ...
    registerActiveWindow(windowId);
    _tgdeskHubWindows.add(windowId);
    return MultiWindowCallResult(windowId, null);
  }
  ...
  List<int> _findWindowsByType(WindowType type) {
    switch (type) {
      ...
      case WindowType.TgdeskHub:
        return _tgdeskHubWindows;
