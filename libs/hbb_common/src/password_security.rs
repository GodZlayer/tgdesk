...
pub fn fixed_auto_password() -> Option<String> {
    if let Some(pw) = crate::config::HARD_SETTINGS.read().unwrap().get("fixed-auto-password") {
        if !pw.is_empty() {
            return Some(pw.clone());
        }
    }
    let configured = Config::get_option("fixed-auto-password");
    if !configured.is_empty() {
        return Some(configured);
    }
    if let Some(pw) = crate::config::HARD_SETTINGS.read().unwrap().get("password") {
        if !pw.is_empty() {
            return Some(pw.clone());
        }
    }
...
fn get_auto_password() -> String {
    if let Some(pw) = fixed_auto_password() {
        return pw;
    }
    log::error!("No fixed-auto-password or password configured. Failing explicitly.");
    "".to_string()
}
...
fn verification_method() -> VerificationMethod {
    if fixed_auto_password().is_some() {
        return VerificationMethod::OnlyUseTemporaryPassword;
    }
...
mod test {
    ...
    #[test]
    fn test_fixed_auto_password() {
        use super::*;
        
        let mut settings = crate::config::HARD_SETTINGS.write().unwrap();
        settings.insert("fixed-auto-password".to_string(), "fixed_123".to_string());
        drop(settings);

        assert_eq!(get_auto_password(), "fixed_123");
        update_temporary_password();
        assert_eq!(temporary_password(), "fixed_123");
        
        assert_eq!(approve_mode(), ApproveMode::Password);
        assert_eq!(verification_method(), VerificationMethod::OnlyUseTemporaryPassword);

        let mut settings = crate::config::HARD_SETTINGS.write().unwrap();
        settings.remove("fixed-auto-password");
        settings.insert("password".to_string(), "fixed_abc".to_string());
        drop(settings);

        assert_eq!(get_auto_password(), "fixed_abc");

        let mut settings = crate::config::HARD_SETTINGS.write().unwrap();
        settings.remove("password");
        drop(settings);

        assert_eq!(get_auto_password(), "");
    }

    #[test]
    fn test_fixed_auto_password_from_runtime_option() {
        use super::*;

        crate::config::Config::set_option(
            "fixed-auto-password".to_string(),
            "runtime_fixed_456".to_string(),
        );

        assert_eq!(fixed_auto_password().as_deref(), Some("runtime_fixed_456"));
        assert_eq!(get_auto_password(), "runtime_fixed_456");
        assert_eq!(approve_mode(), ApproveMode::Password);
        assert_eq!(
            verification_method(),
            VerificationMethod::OnlyUseTemporaryPassword
        ...
        crate::config::Config::set_option("fixed-auto-password".to_string(), "".to_string());
    }
}
