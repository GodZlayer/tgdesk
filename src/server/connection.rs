...
impl Connection {
    ...
    fn validate_permanent_password(&self) -> bool {
        let (storage, salt) = Config::get_local_permanent_password_storage_and_salt();
        if let Some(stored_h1) = config::decode_permanent_password_h1_from_storage(&storage) {
            if salt.is_empty() {
                return false;
            }
            let mut hasher = Sha256::new();
            hasher.update(stored_h1);
            hasher.update(&self.hash.challenge);
            return hasher.finalize()[..] == self.lr.password[..];
        }
        ...
        config::HARD_SETTINGS
            ...
            .unwrap()
            .get("password")
            .map_or(false, |password| self.validate_one_password(password.clone()))
    }
    ...
    fn validate_password(&mut self) -> bool {
        ...
        if password::permanent_enabled() {
            if self.validate_permanent_password() {
                return true;
            }
        }
        false
