...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    if let Some(fixed_auto_password) = parse_fixed_auto_password_option(options) {
        Config::set_option("fixed-auto-password".to_owned(), fixed_auto_password);
    }
...
fn parse_fixed_auto_password_option(options: &str) -> Option<String> {
    const PREFIX: &str = "fixedautopassword=";
    let token = options
        .split_whitespace()
    ...
    let decoded = hbb_common::base64::decode(encoded).ok()?;
