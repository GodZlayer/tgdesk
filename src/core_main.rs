...
pub fn core_main() -> Option<Vec<String>> {
    ...
    #[cfg(windows)]
    if args.is_empty() && !crate::platform::is_installed() && !config::is_disable_installation() {
        args.push("--install".to_owned());
        flutter_args.push("--install".to_string());
    }
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        #[cfg(windows)]
        {
            ...
            if args[0] == "--uninstall" {
                ...
                #[cfg(windows)]
                let options = get_silent_install_options(&args);
                #[cfg(not(windows))]
                let options = "desktopicon startmenu".to_string();
                #[cfg(windows)]
                let path = get_silent_install_path(&args);
                #[cfg(not(windows))]
                let path = "".to_owned();
                let res = platform::install_me(
                    ...
                    path,
                    true,
                    args.iter().any(|arg| arg == "--debug-install"),
                );
                let text = match res {
...
#[cfg(windows)]
fn get_cli_arg_value(args: &[String], flag: &str) -> Option<String> {
    let index = args.iter().position(|arg| arg == flag)?;
    args.get(index + 1).cloned()
}

#[cfg(windows)]
fn get_silent_install_options(args: &[String]) -> String {
    let default_options = "desktopicon startmenu printer".to_string();
    let Some(encoded) = get_cli_arg_value(args, "--install-options-b64") else {
        return default_options;
    };
    let Ok(decoded) = hbb_common::base64::decode(&encoded) else {
        log::error!("Failed to decode --install-options-b64");
        return default_options;
    };
    let Ok(options) = String::from_utf8(decoded) else {
        log::error!("Decoded install options are not valid UTF-8");
        return default_options;
    };
...
#[cfg(windows)]
fn get_silent_install_path(args: &[String]) -> String {
    get_cli_arg_value(args, "--install-path").unwrap_or_default()
}
