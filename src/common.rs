...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://tgdevs.pp.ua".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    url.contains("tgdevs.pp.ua/") || url.ends_with("tgdevs.pp.ua")
}
...
pub fn load_custom_client() {
    ...
    apply_workspace_custom_client_defaults();
}

fn apply_workspace_custom_client_defaults() {
    use hbb_common::config::keys;

    *config::APP_NAME.write().unwrap() = "TGdesk".to_owned();

    let mut settings = config::OVERWRITE_SETTINGS.write().unwrap();
    settings.insert(
        keys::OPTION_CUSTOM_RENDEZVOUS_SERVER.to_owned(),
        "168.232.199.161".to_owned(),
    );
    settings.insert(
    ...
    settings.insert(
        ...
        "3LQEwfAFEtffwSx33n9lvFx8U3yZjMrL8N5W29tCcJM=".to_owned(),
    );
}
